"""Densely scan dleg.mp4 to find best frames for the dleg_lev pose:
a deep level-change crouch — hips dropped, knees compressed, body leaning forward.

Per-frame MediaPipe Pose, scored by:
  - hip drop      (hip_mid_y past 0.45 of frame)
  - knee compression (knee_mid_y - hip_mid_y close to 0; legs folded)
  - forward lean  (|head_x - hip_mid_x|)
  - head dropped  (head_y past shoulder_mid_y)

Filters out frames where:
  - vis_min < 0.5
  - feet not on ground (ank_mid_y < 0.70)
  - hips not dropped (hip_mid_y < 0.45)
  - legs still relatively straight (knee_mid_y - hip_mid_y > 0.15)

Saves top-K annotated thumbnails + a contact sheet + a JSON record.

Usage:
  python tools/pose-extract/find_dleg_lev_poses.py [step=2] [top_k=8]
"""
import json
import os
import sys
...
def lev_score(p, vis):
    ...
    hip_drop = hip_mid_y - 0.45
    knee_compression = max(0.0, 0.20 - max(0.0, knee_mid_y - hip_mid_y))
    forward_lean = abs(head_x - hip_mid_x)
    head_drop = max(0.0, head_y - sh_mid_y)

    score = 2.0 * hip_drop + 2.0 * knee_compression + 1.5 * forward_lean + head_drop
    return score, "x"


SCORERS = [
    ("dleglev", lev_score,
     "2*hip_drop + 2*knee_compression + 1.5*forward_lean + head_drop; "
     "filters: vis>=0.5, ank_mid_y>=0.70, hip_mid_y>=0.45, knee-hip<=0.15"),
]
...
def main():
    ...
    out_path = os.path.join(OUT_POSES, f"{TECH}_lev_candidates.json")
